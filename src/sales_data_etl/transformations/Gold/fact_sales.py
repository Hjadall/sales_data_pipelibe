...
@dlt.table(name="fact_sales", comment="Final Gold Fact table joining Silver Orders with Current-State Gold Dimensions")
def fact_sales():
    ...
    customers_df = dlt.read("silver_customers").filter(col("__end_at").isNull())
    products_df = dlt.read("silver_products").filter(col("__end_at").isNull())
